...
function build(){
	...
	#cat $PACKAGE
	{
        ...
        #module light
		{
			LIGHT_CFG_INIT_PARAM_CHECK=$(fjq .modules.light.light_cfg_init_param_check)
        		LIGHT_CFG_PROD_DRIVER_NEED_INIT=$(fjq .modules.light.light_cfg_prod_driver_need_init)
        		LIGHT_CFG_3IN1_SAVE=$(fjq .modules.light.light_cfg_3in1_save)    
        		LIGHT_CFG_REMOTE_ENABLE=$(fjq .modules.light.light_cfg_remote_enable)
        		LIGHT_CFG_UART_ENABLE=$(fjq .modules.light.light_cfg_uart_enable)
		...
		if [ "$LIGHT_CFG_INIT_PARAM_CHECK" = "null" ];then
			LIGHT_CFG_INIT_PARAM_CHECK=0
			fi

		if [ "$LIGHT_CFG_PROD_DRIVER_NEED_INIT" = "null" ];then
			LIGHT_CFG_PROD_DRIVER_NEED_INIT=0
			fi

		if [ "$LIGHT_CFG_3IN1_SAVE" = "null" ];then
			LIGHT_CFG_3IN1_SAVE=0
			fi

		if [ "$LIGHT_CFG_REMOTE_ENABLE" = "null" ];then
			LIGHT_CFG_REMOTE_ENABLE=1
			fi

		if [ "$LIGHT_CFG_UART_ENABLE" = "null" ];then
			LIGHT_CFG_UART_ENABLE=0
			fi
        #module switch
		{
	...
	echo "> [build-step2]-------------------------------------------"
	{
		...
		f_write 'echo "#define     LIGHT_CFG_INIT_PARAM_CHECK         	'$LIGHT_CFG_INIT_PARAM_CHECK'" >> $CONFIG_FILE'
        	f_write 'echo "#define     LIGHT_CFG_PROD_DRIVER_NEED_INIT    	'$LIGHT_CFG_PROD_DRIVER_NEED_INIT'" >> $CONFIG_FILE'
        	f_write 'echo "#define     LIGHT_CFG_3IN1_SAVE                	'$LIGHT_CFG_3IN1_SAVE'" >> $CONFIG_FILE'      
        	f_write 'echo "#define     LIGHT_CFG_REMOTE_ENABLE            	'$LIGHT_CFG_REMOTE_ENABLE'" >> $CONFIG_FILE' 
        	f_write 'echo "#define     LIGHT_CFG_UART_ENABLE          	'$LIGHT_CFG_UART_ENABLE'" >> $CONFIG_FILE' 

		f_write 'echo "" >> $CONFIG_FILE'

            echo 'echo "#define DEFAULT_CONFIG \"{Jsonver:'$MODULE_LIGHT_JSONVER',category:'$MODULE_LIGHT_CATEGORY',module:'$MODULE_LIGHT_MODULE',cmod:'$MODULE_LIGHT_CMOD',dmod:'$MODULE_LIGHT_DMOD',cwtype:'$MODULE_LIGHT_CWTYPE',\
...
gr:'$MODULE_LIGHTL_GR',gg:'$MODULE_LIGHTL_GG',gb:'$MODULE_LIGHTL_GB',crc:'$MODULE_LIGHTL_CRC',}\" ">> $CONFIG_FILE' | sed "s:[a-z]*\:null,::g"  >>  $F_WRITE_PATH 
...
#git_id: 0832c4e674b1f11d06bc9aece582e6f272890fcd
#date: Wed 20 Jan 20:31:33 CST 2021
